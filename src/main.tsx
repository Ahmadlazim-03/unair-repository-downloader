...
const API = (import.meta.env.VITE_API_URL || 'http://127.0.0.1:8787').replace(/\/$/, '');
...
function App() {
  ...
  return <div className="app-shell">
    ...
    <main>
        ...
        <section className="form-card">
          ...
          <form onSubmit={submit}>
            <fieldset disabled={active}>
              ...
              <div className="privacy-note"><ShieldCheck size={18}/><p>Kredensial dikirim ke backend aplikasi untuk login resmi kampus. Aplikasi tidak menyimpannya di database atau browser.</p></div>
      ...
      <div className="connection-bar"><span className={`status-dot ${health ? 'online' : ''}`}/><span>{health ? health.vpn_mode === 'portal' ? 'VPN otomatis di backend Linux' : 'Mode lokal · menggunakan koneksi VPN laptop' : 'Menunggu backend'}</span><span className="connection-detail"><Wifi size={14}/> {health ? 'Backend terhubung' : 'Backend offline'}</span></div>
      <section id="cara-kerja" className="how"><div><span className="eyebrow">ALUR SEDERHANA</span><h2>Dari tautan menjadi arsip.</h2></div><div className="how-steps"><article><span>01</span><h3>Tempel tautan</h3><p>Ambil URL dokumen dari repository UNAIR.</p></article><article><span>02</span><h3>Verifikasi akses</h3><p>Backend menghubungkan VPN dan membuka dokumen dengan akun Anda.</p></article><article><span>03</span><h3>Simpan PDF</h3><p>Halaman diperiksa, diurutkan, lalu disatukan untuk diunduh.</p></article></div></section>
